...
def prep_midi(maestro_root, output_dir):
    ...
    if 'v2.0.0' in maestro_root:
        ...
        maestro_csv_file = os.path.join(maestro_root, csv_file)
        if(not os.path.isfile(maestro_csv_file)):
            print("ERROR: Could not find file:", maestro_csv_file)
            return False
    
        maestro_csv = pd.read_table(maestro_csv_file, delimiter =',')
        print("Found", len(maestro_csv), "pieces")
        print("Preprocessing...")
        ...
        for i_row, row in maestro_csv.iterrows():
            mid         = os.path.join(maestro_root, maestro_csv.loc[i_row, 'midi_filename'])
            split_type  = maestro_csv.loc[i_row, 'split']
            f_name      = mid.split("/")[-1] + ".pickle"
            ...
            total_count += 1
            if(total_count % 50 == 0):
                print(total_count, "/", len(maestro_csv))
