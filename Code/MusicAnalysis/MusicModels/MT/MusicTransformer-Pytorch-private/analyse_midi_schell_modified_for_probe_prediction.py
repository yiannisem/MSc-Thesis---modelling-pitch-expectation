...
dir_midi = os.path.join(dir_cur, 'schellenberg_midi_stimuli_with_probe')
...
for len_context in len_context_list:
    fn_out = f'rpr_ftuned_schell_{model_tag}_all_probs.csv'
    fn_out_raw = f'rpr_ftuned_schell_{model_tag}_all_probs_raw.csv'
    ...
    if len(probs_rows) > 0:
        ...
        pd.DataFrame(raw_probs_norm, columns=headers).to_csv(
            os.path.join(dir_out, f"rpr_ftuned_schell_{model_tag}_all_probs_norm.csv"),
            index=False
        )
    ...
    if len(probe_rows) > 0:
        ...
        out_probe = os.path.join(dir_out, f"rpr_ftuned_schell_{model_tag}_probe_probs.csv")
